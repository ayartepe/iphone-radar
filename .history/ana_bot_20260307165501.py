...
# Taranacak modelleri buraya ekledik (İstediğini ekleyip çıkarabilirsin)
MODELLER = [
    "iphone 16 pro max", "iphone 16 pro", "iphone 16 plus", "iphone 16",
    "iphone 15 pro max", "iphone 15 pro", "iphone 15 plus", "iphone 15",
    "iphone 14 pro max", "iphone 14 plus", "iphone 14",
    "iphone 13 128gb", "iphone 13 256gb", "iphone 11"
]
...
def fiyat_kontrol_ve_kaydet(urun_adi, yeni_fiyat):
    conn = sqlite3.connect('fiyat_takip.db')
    c = conn.cursor()
...
def amazon_tara(model):
    # Model ismini URL'ye uygun hale getiriyoruz
    query = model.replace(" ", "+")
    ...
    try:
        ...
        for item in items[:3]: # Her model araması için en üstteki 3 sonucu al
            name_tag = item.find('h2')
            if not name_tag: continue
            name = name_tag.get_text(strip=True)[:60]
            price_whole = item.find('span', {'class': 'a-price-whole'})
            if price_whole:
                price = int(price_whole.get_text(strip=True).replace(".", "").replace(",", ""))
                if fiyat_kontrol_ve_kaydet(name, price):
                    bot.send_message(CHAT_ID, f"🏗 *AMAZON İNDİRİMİ*\n\n📱 {name}\n💰 Yeni Fiyat: {price:,} TL", parse_mode="Markdown")
    except Exception as e: print(f"Amazon ({model}) Hatası: {e}")

def cimri_tara(model):
    query = model.replace(" ", "+")
    ...
    except Exception as e: print(f"Cimri ({model}) Hatası: {e}")

db_kur()
while True:
    print(f"[{time.strftime('%H:%M:%S')}] Kapsamlı piyasa taraması başlıyor...")
    
    for model in MODELLER:
        print(f"-> {model} taranıyor...")
        amazon_tara(model)
        cimri_tara(model)
        time.sleep(5) # Banlanmamak için kısa bir mola
        
    print("Bütün modeller tarandı. 1 saat uyku moduna geçiliyor.")
    time.sleep(3600)